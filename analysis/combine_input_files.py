...
df_1 = chunk_and_filter('output/joined/full/input_v2_2020-03-01.csv.gz')

# all files in output that start with input and end with .csv.gz and don't contain 'end' and are not input_2020-04-01.csv.gz
files = [f for f in os.listdir('output/joined/full') if f.startswith('input_v2') and f.endswith('.csv.gz') and not f.startswith('input_v2_2020-03-01') and not f.endswith('end.csv.gz')]
print(files)
for file in files:
...
df_1.to_csv('output/joined/full/input_v2_all_py.csv.gz', index=False)
